...
def main(args):
    ...
    with tqdm(total=args.optimization_steps, dynamic_ncols=True) as pbar:
        for x, labels in islice(cycle(train_loader), args.optimization_steps):
            ...
            if do_log:
                train_losses.append(
                    compute_loss(mlp, train, args.loss_function, device, N=len(train))
                )
                train_accuracies.append(
                    compute_accuracy(mlp, train, device, N=len(train))
                )
                test_losses.append(
                    compute_loss(mlp, test, args.loss_function, device, N=len(test))
                )
...
if __name__ == "__main__":
    ...
    # Grokfast
    parser.add_argument(
        "--filter", type=str, choices=["none", "ma", "ema", "kalman"], default="none"
    )
